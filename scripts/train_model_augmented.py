...
class ImageFeatureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx < metadata_df.shape[0]:
            images = video_features_path+self.metadata_df.iloc[idx2]['frames_filename']+'.pt'
        else:
            # idx2 = idx % metadata_df.shape[0] # 0-4
            i = math.floor(idx/metadata_df.shape[0])-1
            images = video_features_aug_path+self.metadata_df.iloc[idx2]['frames_filename']+"_batch"+str(i)+'.pt'         
        # images = torch.load(video_features_path+self.metadata_df.iloc[idx]['frames_filename']+'.pt').to(device)
        label = self.metadata_df.iloc[idx2]['class']
        label_idx = classes_idx[label]
        video_len = self.metadata_df.iloc[idx2]['frame_count']
